let S = {
    init: function () {
        let action = window.location.href, i = action.indexOf('?a=');
...
S.UI = (function () {
    let input = document.querySelector('.ui-input'), ui = document.querySelector('.ui'), help = document.querySelector('.help'), commands = document.querySelector('.commands'),
        overlay = document.querySelector('.overlay'), canvas = document.querySelector('.canvas'), interval, isTouch = false, currentAction, resizeTimer, time, maxShapeSize = 30,
        firstAction = true, sequence = [], cmd = '#';
...
S.Dot.prototype = {
    clone: function () {
        ...
        let dx = this.p.x - n.x, dy = this.p.y - n.y, d = Math.sqrt(dx * dx + dy * dy);
...
S.ShapeBuilder = (function () {
    ...
    return {
        imageFile: function (url, callback) {
            let image = new Image(), a = S.Drawing.getArea();
            ...
            let dots = [], width = gap * w, height = gap * h;
...
S.Shape = (function () {
    ...
    return {
        shuffleIdle: function () {
            ...
            let size, a = S.Drawing.getArea();
            width = n.w;
            height = n.h;
            compensate();
            if (n.dots.length > dots.length) {
                size = n.dots.length - dots.length + 100;
                for (let d = 1; d <= size; d++) {
                    dots.push(new S.Dot(a.w / 2, a.h / 2));
                }
            }
            let d = 0, i = 0;
            console.log('=====', n.dots.length, dots.length);
